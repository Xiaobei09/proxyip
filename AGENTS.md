...
1. 脚本模块级 docstring（第一行 `"""..."""`）是否准确？
2. `docs/scripts.md` CLI 参考是否需要更新（参数、输出文件、行为变化）？
3. `README.md` 目录结构是否需要更新（新增/删除文件）？
4. `DEVELOPMENT.md` 脚本速查是否需要更新（新增命令、参数表）？
5. 新增脚本是否需要新增测试文件？

检查清单：**修改 → 测试 → 语法检查 → 文档更新 → 提交**
