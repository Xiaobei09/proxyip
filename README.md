...
- **流程**：跑测试（`unittest`）→ 下载整理（上游 `all.json`，失败回退 zip；另并 22 个第三方 CF 反代 / 免费 `ip:port` 补充源，完整清单见 `docs/scripts.md`「CF 反代补充来源」；产出 `data/quality/upstream_meta.json`）→ 验证与测速（`--time-budget 3600`，上一轮未存活的条目先经 2 秒 TCP 预连通筛除朽尸）→ 有变更则自动提交并推送回仓库
...
- **流程**：跑测试（`unittest`）→ `china_check.py`（对 `data/valid/all.txt` 全量池，`--limit 0`，全免费 CN 验证源分层判定：itdog 批量 http + batch_tcping 大节点池降级 + batch_ping ICMP 兜底（CN-26）、check-host.cc 呼和浩特 TCP（fail 追加同节点 ICMP 消歧，ok 且孤证追加 HTTPS 应用层确认）/xxapi.cn 北京 TCP＋枣庄 ICMP＋状态码＋443 扫描/jkapi 宁波电信 TCP（附电信 isp_ms）+ICMP+TLS 三协议单节点（含 api.jkapi.com 镜像 failover），多源复核集：tcptest.cn（800 键/20 并发）＋tcptest-ping（400 键/20 并发，同站 ICMP）＋tcptest-http（200 键/8 并发，同站应用层）＋tcptest-trace（200 键/6 并发，同站路由追踪）、ip.net.coffee（1200/40）、pingloc（600/12）、antping（500/8）、antping-ping（200 键/8 并发，同站 ICMP）、tcping.cn（400 键/8 并发，ALTCHA 会话复用）＋tcpingcn-ping（200 键/6 并发，同通道 ICMP）、chinaz（200/8）、98ce.com（200 键/6 并发，34 大陆省运营商节点 TCPing）＋ce98-ping（200 键/6 并发，同站 ICMP）、biuping.com（200 键/8 并发，约 39 ISP×节点 TCPing 测量单元）＋biuping-ping（200 键/8 并发，同站 ICMP）、aa1ping（200 键/6 并发，28 城三网 TCPing，独立运营商）、ping.pe（300））→ `annotate_classify.py`（填充缺失后缀 + 追加分类 token）→ 有变更则自动提交并推送；完成后再由专职 build-good 与 stats 工作流重建 good 清单/图表（含 CN 数据）
