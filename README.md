...
- **流程**：跑测试（`unittest`）→ `china_check.py`（对 `data/valid/all.txt` 全量池，`--limit 0`，全免费 CN 验证源分层判定：itdog 批量 http + batch_tcping 大节点池降级 + batch_ping ICMP 兜底（CN-26）、check-host.cc 呼和浩特/xxapi.cn 北京/jkapi 宁波电信 TCP+ICMP 双协议单节点（含 api.jkapi.com 镜像 failover），多源复核集：tcptest.cn（800 键/20 并发）、ip.net.coffee（1200/40）、pingloc（600/12）、antping（500/8）、antping-ping（200 键/8 并发，同站 ICMP）、tcping.cn（400/8）、chinaz（200/8）、98ce.com（200 键/6 并发，34 大陆省运营商节点 TCPing）、biuping.com（200 键/8 并发，约 39 ISP×节点 TCPing 测量单元）、aa1ping（200 键/6 并发，28 城三网 TCPing，独立运营商）、ping.pe（300））→ `annotate_classify.py`（填充缺失后缀 + 追加分类 token）→ 有变更则自动提交并推送；完成后再由专职 build-good 与 stats 工作流重建 good 清单/图表（含 CN 数据）
