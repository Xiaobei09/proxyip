...
| sorbs | 5 | SORBS `dnsbl.sorbs.net` 社区 open-proxy 实时 DNSBL：SOCKS/HTTP 代理码 `127.0.0.2`/`127.0.0.7` → `listed` 扣 30；动态住宅段 `127.0.0.4/8/9` 忽略（与 spamhaus PBL 同口径）；复用 dnsbl 的 DoH/sticky/并发与负缓存；opt-in 不入默认；上限 SORBS_CAP=9000/轮。**R271 新增**。**R272 实测**：test-point `2.0.0.127.dnsbl.sorbs.net` 经 DoH 无响应（不断言死亡，保持 opt-in 待验证；若长期零命中且确认停服则注释停用） |
