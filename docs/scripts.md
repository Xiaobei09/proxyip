...
除主源外，默认还会拉取一批 **自称 Cloudflare 第三方反代 proxyip 的来源**（非官方 CF 段）并合并：`wentao883/TG-wxgqlfx_ZBDW`（`fdip`/`vlid`/`yxip`，`plain`）、`ChatBotPlus/cf-proxyips`（`plain`）、`ymyuuu/IPDB`（`BestProxy/proxy.txt`、`bestproxy&country.txt`、`bestproxy.txt`，`ip`）、`mountain787/Lunch-Bag-ip`（`csv`）、`LeilaoMi/cf-proxyip-us`（`docs/all.txt`，逐台人工/脚本实测收录的非 CF 反代池，同样排除官方 CF 段）、`ipdb.api.030101.xyz`（`?type=proxy` 全量反代池 + `?type=bestproxy` 优选反代池 + `?type=bestproxy&country=true` 带国家标注的优选池，`ip#CC` 元数据经 `extract_bare_ips` 解析并按 443 归位，同源族、互为补充，扩大可用性）、`Wwuyi123/CF-Proxyip`（`proxyip.txt`、`proxyip_with_country.txt`（`IP#速度地区`）、`ips/all_ips.txt`，`ip`；IP-02 起加 `Manual_input_IP.txt` 手工精选裸 IP，多国别 VPS）、`wanwushequ/ProxyIP`（`US.txt`/`JP.txt`/`HK.txt`/`SG.txt` 等地区优选榜，`ip`；IP-01 起纳入香港榜，IP-03 起纳入新加坡榜）、`svip-s/cloudflare_ip`（`best_ips.txt`，`ip:port#CC [注解]`，`ipnote`；实测 IP 属第三方反代 VPS、非 CF 官方 AS13335 段，与池政策「非 AS13335」一致）。解析方式分五种：`plain`（`ip:port#国家`/`ip:port#中文`）、`ip`（裸 IP，统一按 443 端口）、`csv`（`IP,端口,地区,延迟`，地区为机场码或国家码）、`json`（`all.json` 格式镜像，缺失国家字段的条目归入 `ALL`，畸形载荷容忍）、`ipnote`（`ip:port#CC [注解]` 榜单行，CC 为两位字母，尾部方括号注解或旗标一并剥离，适用 LancelotRar/svip-s 等榜单格式）。
...
| `spamcop` | 6 worker、0.15s | （上限 9000/轮，bl.spamcop.net 社区实时 DNSBL） |
| `sorbs` | 6 worker、0.15s | （上限 9000/轮，`dnsbl.sorbs.net` opt-in DNSBL，SOCKS/HTTP 代理码 127.0.0.2/7——**R271 新增 opt-in 源；R274 起 pacing 与其余 DNSBL 归一 0.15s**） |
| `dronebl` | 6 worker、0.15s | （上限 9000/轮，dnsbl.dronebl.org 僵尸/失陷主机社区 DNSBL，命中码 2~13 均判 listed，复用 dnsbl 同一 DoH/负缓存/sticky 通路） |
| `spamrats` | 6 worker、0.15s | （上限 SPAMRATS_CAP=9000/轮，dnsbl.spamrats.com 社区双通路 DoH，A 码 127.0.0.2/3 → listed，**DYN 127.0.0.4 忽略**，复用 dnsbl sticky/负缓存——**R270 新增 opt-in 源**；R272 实测 test-point 无响应，保持 opt-in 待验证） |
| `uceprotect` | 6 worker、0.15s | （上限 UCEPROTECT_CAP=9000/轮，dnsbl-1.uceprotect.net L1 社区发送者黑名单 DoH，A 码仅 127.0.0.2 → listed，L2/L3 不用，复用 dnsbl sticky/负缓存——**R272 新增 opt-in 源**，test-point `2.0.0.127` 实测回包 `127.0.0.2`） |
| `psbl` | 6 worker、0.15s | （上限 PSBL_CAP=9000/轮，psbl.surriel.com 被动垃圾名单 DoH，A 码仅 127.0.0.2 → listed，复用 dnsbl sticky/负缓存——**R273 新增 opt-in 源**，test-point 实测回包 `127.0.0.2`） |
...
- **L2 itdog.cn 批量实测（主源）**：每任务 5 目标 × 电信/联通/移动各 6 节点（共 18 节点，`ITDOG_NODES_PER_ISP=6`），经 WebSocket 收结果，TCP 连通即判可达；节点按运营商归属（id 映射兜底节点名关键词），任务级另出 per-ISP 最小 RTT（`isp_ms`），落 china.json 后供 CN 清单取**最快运营商视角**（`common.cn_fastest_ms`）渲染展示延迟与 `≈XMB/s` 速度。batch_http 失败/被限的键另有两条同站降级：`batch_tcping`（纯 TCP，大节点池）记 `itdog_tcping` 源；`batch_ping`（CN-26，ICMP，大节点池电信 87/联通 83/移动 89）记 `itdog_ping` 源（归一 `level=icmp`、不产 `isp_ms`）
- **L2 单节点实测（并发）**：`check-host.cc`（呼和浩特阿里云节点，匿名限速 5/10s、250/h，配置 key 可放宽；CN-31 起 TCP-fail 追加同节点 ICMP ping 消歧，CN-32 起 TCP-ok 且其余免额 0 ok 追加同节点 HTTPS 应用层确认，共用配额）+ `xxapi.cn`（北京节点 TCP，免 key）+ `xxapi.cn/api/ping`（CN-29：山东枣庄 BGP 节点 ICMP，免 key，`level=icmp`，echo 校验，不产 `isp_ms`）+ `xxapi.cn/api/status`（CN-38：同站 HTTP 状态码，免 key，`level=http`，`-2` 判 fail，无 ms）+ `xxapi.cn/api/portscan`（CN-39：同站 8 端口扫描，仅 443 键产出，`level="tcp"` 布尔见证）+ `jkapi.com/zz_tcping`（浙江宁波电信 TCP，免 key；CN-39 起 ok 附 `isp_ms={中国电信}`）+ `jkapi.com/zz_ping`（CN-25 新增：同站同节点 ICMP 主机存活，免 key，`level=icmp`，不进延迟显示/不产 `isp_ms`，与 chinaz/coffee 同口径；CN-26 起 jk 三端点主站异常自动 failover 同站镜像 `api.jkapi.com`，429 不切换）+ `jkapi.com/zz_ssl`（CN-37 新增：同站同节点 TLS 握手，免 key，`level="tcp"` 保守，无 ms 只作布尔见证）。**保守判定：多节点源（pingpe/itdog/tcpping/itdog_tcping/itdog_ping/tcptest/tcptest_ping/tcptest_http/coffee/pingloc/antping/antping_ping/tcpingcn/tcpingcn_ping/chinaz/ce98/ce98_ping/biuping/biuping_ping/aa1ping/boce/ipip/17ce/ping0/wansui，与 `merge_verdict` 的 `multi_ok` 表一致）单独确认 → reachable；单节点源（check_host/checkhost_ping/checkhost_http/xxapi/xxping/xxstatus/xxscan/jkapi/jkping/jkssl）≥2 个确认 → reachable；仅 1 个确认 → uncertain；均失败 → unreachable**
- **L3 多节点复核（有界并发小样本）**：`tcptest.cn`（免费 REST，~146 大陆节点按运营商均衡采样 10 个，TCP `ip:port` 直连，节点成功率达 50% 即判可达）先于 ping.pe 跑——免费、端到端 ~2-6s/键，确认过的键自动让位；`tcptest_ping`（CN-33：同站 `type=ping`，裸 IP 目标，`success`＋`avg_ms>0`，`level=icmp`，不产 `isp_ms`，与 TCP 共用节点采样跑在 TCP 相之后）；`tcptest_http`（CN-35：同站 `type=http`，`http://ip:port/`，`success`＋`status>0` 即应用层确认，`level=http`，ms 取 `connect_ms`，与 TCP 同口径产 `isp_ms`，跑在 ping 相之后）；`tcptest_trace`（CN-40：同站 `type=traceroute`，裸 IP 目标，`success` 即达判，`ms` 恒空，`level=icmp`，不产 `isp_ms`，跑在 http 相之后）；`ip.net.coffee`（18 ICMP 节点，成功率达 50% 判可达，专测中国大陆主机存活）；`pingloc.com`（~12 节点 ICMP ping，纯 HTTP+SSE 零鉴权）；`antping.com`（~155 节点，JWT+WS，ICMP ping / TCP `ip:port` 均可）；`tcping.cn`（~163 TCP 节点，SHA-256 PoW + ALTCHA 会话复用纯 Python 求解 + WS，真实端口直连；CN-30 起同通道 `tcpingcn_ping` ICMP 并行，`level=icmp`，紧凑键 `r/q` 判定，不产 `isp_ms`）；`ping.chinaz.com`（~53 ICMP 节点，服务端渲染 token + WS）；`98ce.com`（34 个大陆各省运营商节点持续 TCPing，socket.io v4 over WebSocket，CF 反爬用 HTTPS+Referer 壳页取节点、连 `wss://www.98ce.com/socket.io` 发/收事件，零 key，实测 35/35 节点出数）；`ce98_ping`（CN-36：同站 continuous-ping，35 节点 socket.io，事件与 TCP 全同形，`level=icmp`，不产 `isp_ms`，活体 35/35 出数）；`biuping.com`（约 39 个 ISP×节点 TCPing 测量单元，纯 HTTP + SSE、CSRF token 从壳页 meta 提取，零 key，实测 39/39 出数）；`biuping_ping`（CN-34：同站 ICMP，复用 port="" 分支，`level=icmp`，剥离 `isp_ms`，活体 39/39 出数）；`ping.aa1.cn`（CN-27：独立运营商免费 API 站，WS 纯 JSON 零鉴权，28 城三网 TCPing，`domain` 携真实端口逐端口实测，`operator` 原生分 ISP，实测 28/28 出数）；`antping_ping`（CN-28：antping 同站 ICMP，复用 code=3 分支，`level=icmp`，不产 `isp_ms`，活体 178/179 出数）；随后 `ping.pe`（约 13 个大陆节点，≥7/13 可达即判可达，报告不足 5 节点 → inconclusive），各源均只投「当前尚未被 itdog/单节点源判可达」的键且按 `--<name>-limit` 有界；多节点源须「≥ `MULTI_MIN_NODES`（5）个节点 + 成功率达标」才可独立判 reachable，防限流残缺样本假阳性；可选 `tcpping.cn`（多运营商，需 `TCPPING_CN_TOKEN`，缺 key 自动跳过）
...
| `--tcptest-concurrency` | tcptest.cn 并发复核数 | 8 |
| `--tcptest-nodes` | tcptest.cn 每键采样节点数 | 10 |
| `--tcptest-ping-limit` | tcptest.cn ICMP 多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--tcptest-ping-concurrency` | tcptest.cn ICMP 并发复核数 | 8 |
| `--tcptest-http-limit` | tcptest.cn HTTP 应用层多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--tcptest-http-concurrency` | tcptest.cn HTTP 并发复核数 | 8 |
| `--tcptest-trace-limit` | tcptest.cn 路由追踪多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--tcptest-trace-concurrency` | tcptest.cn 路由追踪并发复核数 | 8 |
...
| `--tcpingcn-ping-limit` | tcping.cn ICMP 多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--tcpingcn-ping-concurrency` | tcping.cn ICMP 并发复核数 | 6 |
...
| `--ce98-ping-limit` | 98ce.com ICMP socket.io 多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--ce98-ping-concurrency` | 98ce.com ICMP 并发复核数 | 6 |
| `--biuping-limit` | biuping.com SSE 多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--biuping-concurrency` | biuping.com 并发复核数 | 8 |
| `--biuping-ping-limit` | biuping.com ICMP 多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--biuping-ping-concurrency` | biuping.com ICMP 并发复核数 | 8 |
| `--aa1ping-limit` | ping.aa1.cn WS 多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--aa1ping-concurrency` | ping.aa1.cn 并发复核数 | 6 |
| `--antping-ping-limit` | antping.com ICMP 多节点复核条数（0=跳过；-1=全部未定键） | 0 |
| `--antping-ping-concurrency` | antping.com ICMP 并发复核数 | 8 |
...
### `scripts/china_itdog.py`（库模块）

itdog.cn 批量大陆可达性探测（WebSocket 收结果）：自 `china_check` 拆出的独立库模块（无 `__main__`，供 `china_check.py` import 复用）——抓取电信/联通/移动节点 → 提交批量 HTTP/TCPing 任务 → 经 `wss://www.itdog.cn` 轮询收集 → 聚合成源判定。仅依赖 `common` 的 `UA`/`request_follow`；8 次连续批量失败触发断路器暂停，任务带 pacing + 并发上限。参数经 `china_check.py` 透传生效。
