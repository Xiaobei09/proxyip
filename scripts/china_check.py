...
# R242 注：本判据刻意**比通道命名空间宽松**（``cn\d{1,2}`` 会连视图家族后缀
# ``cn4``/``cn6``/``cn46`` 一起匹配）。这是**有意的纵深防御**、且行为可证无害：
# ``sub`` 只在 ``cn_public_id`` 能解出 id 时才替换，而后者对家族后缀与越界值
# （实测 ``cn4``/``cn6``/``cn46``/``cn``/``cn45`` 全返回 ``None``）一律拒绝。
# 即：真正的开关是 ``cn_public_id`` 的注册表校验，本正则只是**候选筛子**——
# 筛子放宽不改变结果，筛子收紧反而可能因漏掉形态而漏替换。故保持现状。
