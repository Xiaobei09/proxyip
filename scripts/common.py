...
def _fmt_cn_speed(value: float) -> str:
    """渲染 CN 视角估算速度：**统一两位小数**（R247，用户裁决「按契约」）。

    两段历史：

    - **R221**：原实现一律 ``round(v, 1)``，于是 0.01 → ``0.0``——把一个真实
      低速节点发布成「速度为零」，违反数据契约 A「测速失败省略速度段**不写 0**」。
      当时加了精度阶梯：正数若一位小数会归零则退两位，其余仍一位。
    - **R247**：那个阶梯本身是**新引入的缺陷**——同一段里 **99.6% 一位小数、
      仅 sub-0.1 两位**，即**混合精度**；而 ``docs/data-spec.md`` 第 69 行
      （测速段）与第 214 行（JSON 导出）都写明速度是「**两位小数**」。
      代码与成文契约不一致，且契约是唯一权威。R222 当时已诊断出这一点并
      改好了函数，但因 8 条断言仍期望一位小数、受「不得删/弱化断言」约束
      而未收口，其工作又存进 ``git stash``（非 push 保护）**已丢失**。
      本轮按用户裁决重建。

    故：**无条件两位小数**。真正为 0 的输入仍走删除路径（见
    :func:`_rewrite_cn_speed`）——本函数不负责那个判断。

    影响面（实测）：CN 估算段约 2.6 万个 ``≈`` token 中 99.6% 为一位小数，
    改后**数值一个都不变**，只是补零（``≈2.0`` → ``≈2.00``）；已发布数据
    随下次重跑自然对齐，无需手改。
    """
    return f"{value:.2f}"
