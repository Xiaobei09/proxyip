...
```
scripts/
  common.py              共享模块：data 布局常量、行/HTTP/JSON 助手、request_follow、CONNECT 探测
  download_proxies.py    阶段 1：下载（all.json + zip 回退）、解压、整理、去重、差异归档、上游元数据
  validate_proxies.py    阶段 2：存活验证 + 测速（asyncio）
  quality_check.py       阶段 3 入口：流媒体解锁 + 出口 IP 质量 + 多源信誉（facade）
  quality_reputation.py  阶段 3 子模块：信誉分/滥用分（quality_check 拆分）
  quality_streaming.py   阶段 3 子模块：流媒体解锁（quality_check 拆分）
  generate_stats.py      阶段 4：统计与 SVG 图表
  china_check.py         独立 CI：大陆连通性检测（CF 启发式 + check-host + xxapi + ping.pe）
  china_itdog.py         独立 CI 子模块：itdog.cn 批量探活（china_check 拆分）
...
docs/                    用户文档拆分：data-spec.md（数据规范）/ scripts.md（CLI 参考）
archive/Check_Proxy.js   遗留的 Cloudflare Worker 单节点检查脚本（已停用归档）
README.md                用户文档概览（数据规范与 CLI 见 docs/）
AGENTS.md                AI 协作硬性规则（必提交必推送，本地未跟踪）
DEVELOPMENT.md           本文（本地未跟踪）
```
...
- 提交身份 `Xiaobei09 <235415509+Xiaobei09@users.noreply.github.com>`（本地 config 已设）。GitHub 认证走 `gh` credential helper。
- **铁律**：不删除对象库内 `.l2s.*` 文件（真实对象可能是符号链接指向它们的后备）；不在对象目录做跨盘 `mv`/`cp -a`；不手动重建 git 目录（见 §9 排查）。

## 4. 测试与检查

```bash
python3 -m unittest discover -s tests          # 全部测试（当前 298 个，须全绿）
...
python3 scripts/china_check.py --help          # 独立 CI：大陆连通性（勿直接全量跑）
python3 scripts/exit_family.py --help          # 独立 CI：实际出口家族（勿直接全量跑）
python3 scripts/generate_stats.py --help       # 阶段 4（勿直接跑，会改 data/）
```

- `quality_check.py` 现为 facade：逻辑在 `quality_reputation.py` / `quality_streaming.py`，新增信誉/流媒体逻辑请放入对应子模块；`qc.*` 符号经 `import *` 重新导出，测试对 `qc` 的补丁需打在**属主模块**（如 `quality_reputation.netcoffee_lookup_sync`）上。
- `china_check.py` 的 itdog 逻辑已拆到 `china_itdog.py`（`cc.*` 同理）。
- 共享常量与助手一律放 `common.py`，新脚本不得再从其它入口脚本 import。
...
python3 scripts/annotate_classify.py --help     # 后缀填充 + 节点分类
```
...
分层判定：L1 启发式 CF（零网络）→ L2 itdog 批量（电信/联通/移动各 1 节点）+ check-host.cc + xxapi.cn（并发实测）→ L3 ping.pe/tcpping.cn（多节点复核）。任一实测源成功即判可达；check-host 与 xxapi 均失败才判不可达；单源失败归 uncertain。key 走环境变量 `CHINA_CHECK_API_KEY`、`TCPPING_CN_TOKEN`。CI 全量池（`--source data/valid/all.txt --limit 0`）；产出 `china.json`、`all_cn.txt`（全量大陆可达清单，源为 all.txt）并向 `all.txt`/`all_ltd.txt` 追加 `-CN`。详见 README「大陆连通性检测」章节。
...
### annotate_classify.py 关键参数

| 参数 | 默认 | 说明 |
|---|---|---|
| `--data-dir` | `data` | 数据根目录（含 `valid/` 子目录） |

后缀填充 + 节点分类（CI 在 quality-check 完成后自动运行）。读取 `ipinfo.json`、`reputation.json`、`china.json`、`exit_family.json`、`streaming.json` 五个 JSON 数据源，向所有 `data/valid/*.txt` 文件填充缺失后缀（CN/V4/V6/streaming/rep）并追加分类 token（IP 类型 DC/RES/MOB/PROXY + 速度等级 fast/mid/slow）。幂等：多次运行结果一致。详见 `docs/scripts.md`。
...
| 日期 | 提交 | 说明 |
|---|---|---|
| 2026-08-16 | `refactor: split itdog logic out of china_check.py` | 拆分 `china_itdog.py`（itdog_* + _WebSocket + _pace + ITDOG_* 常量），china_check 经 `from china_itdog import *` 复用；cc._note/is_cf_heuristic/request_follow 统一走 common |
| 2026-08-16 | `refactor: split quality_check.py into reputation/streaming modules` | 拆 `quality_reputation.py` / `quality_streaming.py`，quality_check 变 facade（`import *` 重新导出 58 个 def）；测试对跨模块函数打补丁须打在属主模块（qr.*） |
| 2026-08-16 | `refactor: extract scripts/common.py, drop cross-entry imports` | 新增共享模块（data 布局常量 + 行/HTTP/JSON 助手 + request_follow/_note/is_cf_heuristic/try_connect），入口脚本不再互相 import；china_check 本地 UA 保留（与 common 值不同） |
